...
\label{subsec:data-layer-architecture}
...
\subsubsection{Conceptual Design (ERD)}
...
\paragraph{Core Entity Relationships}
...
\paragraph{The Dynamic Credential Architecture}
...
\subsubsection{Mapping to Relational Model (Logical/Physical Design)}
...
\paragraph{Primary and Foreign Key Strategy}
\begin{itemize}
...
\paragraph{Junction and Mapping Tables}
...
\paragraph{Intentional Denormalization (JSON Workflows)}
...
\subsubsection{Physical Database Design \& Implementation}
...
\paragraph{Database Engine: PostgreSQL (via Supabase)}
...
\paragraph{Migration Management}
...
\paragraph{UserCredentialSet Table}
